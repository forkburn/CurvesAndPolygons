...
import com.warfactory.curvesandpolygons.thread.OldTimesScrRunnable;

public class OldTimesScrWallpaperService extends WallpaperService {
	...
	class OldTimesScrEngine extends Engine implements SharedPreferences.OnSharedPreferenceChangeListener {
		...
		private OldTimesScrRunnable mScrRunnable;
		...
		public OldTimesScrEngine() {
			...
			mScr.setCurrentRenderMode(getRenderModePref());
		}
		...
		public void startRendererThread(){
			mScrRunnable = new OldTimesScrRunnable(mSurfaceHolder, mScr);
		...
		public void onSharedPreferenceChanged(SharedPreferences pref, String key) {
			...
			}else if (key.equals(getString(R.string.pref_render_mode_key))){
				mScr.setCurrentRenderMode(getRenderModePref());
		...
		private MovingLoopsRenderer.RenderMode getRenderModePref () {
			// apply the render mode setting
			String renderModeKey = getString(R.string.pref_render_mode_key);
			String renderMode = mPrefs.getString(renderModeKey, "");
			String[] validRenderModes = getResources().getStringArray(R.array.pref_render_mode_value); 

			MovingLoopsRenderer.RenderMode result = MovingLoopsRenderer.RenderMode.STROKE_ONLY;

			if (renderMode.equals(validRenderModes[0])){
				result = MovingLoopsRenderer.RenderMode.SOLID_COLOR;
			}else if (renderMode.equals(validRenderModes[1])){
				result = MovingLoopsRenderer.RenderMode.STROKE_ONLY;
			}
			return result;
		}
